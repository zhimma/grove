...
func TestRootCommandIncludesArtisanHelp(t *testing.T) {
	cmd := newRootCmd()
	var out bytes.Buffer
	cmd.SetOut(&out)
	cmd.SetErr(&out)
	...
	content := out.String()
	assertContains(t, content, "artisan 是当前仓库唯一保留的 CLI 入口")
...
`)
	...
	service := mustRead(t, filepath.Join(root, "app/console/service/product_category.go"))
	...
	assertContains(t, service, "pkgerrors.ServiceUnavailable")

	model := mustRead(t, filepath.Join(root, "internal/model/product_category.go"))
	assertContains(t, model, `return "product_categories"`)

	handler := mustRead(t, filepath.Join(root, "app/console/handler/product_category.go"))
