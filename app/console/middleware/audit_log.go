...
func AuditOperation(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if auditMeta := reqctx.GetAuditMeta(c); strings.TrimSpace(auditMeta.TargetType) != "" || strings.TrimSpace(auditMeta.TargetID) != "" || len(auditMeta.Detail) > 0 {
