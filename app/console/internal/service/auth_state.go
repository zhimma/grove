...
func (r *adminAuthStateResolver) ResolveAdminAuthState(ctx context.Context, adminID string) (*AdminAuthState, error) {
	...
	if err := r.dbRepo.Default().WithContext(ctx).
		...
		if err == gorm.ErrRecordNotFound {
			return nil, errx.Unauthorized().WithMessage("未登录或登录已失效")
		}
		return nil, errx.Internal().WithCause(err)
	}

	if !admin.CanLogin() {
		return nil, errx.Forbidden().WithMessage("账号已被禁用").WithCode("account_disabled")
	}
	if admin.RoleID != "" && (admin.Role == nil || !admin.Role.IsActive()) {
		return nil, errx.Forbidden().WithMessage("角色不可用").WithCode("role_unavailable")
	}
