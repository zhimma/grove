...
func NewAuthService(dbRepo database.Repo, enforcer *pkgcasbin.Enforcer, tm *auth.Manager) *AuthService {
	return &AuthService{
		dbRepo:       dbRepo,
		enforcer:     enforcer,
		tokenManager: tm,
...
func (s *AuthService) Login(ctx context.Context, input LoginInput) (LoginOutput, error) {
	if s.dbRepo == nil || s.dbRepo.Default() == nil {
		return LoginOutput{}, pkgerrors.ServiceUnavailable().WithMessage("默认数据库未配置")
	}
	if s.tokenManager == nil {
		return LoginOutput{}, pkgerrors.ServiceUnavailable().WithMessage("令牌管理器未配置")
	}

	account := strings.TrimSpace(input.Account)
	if account == "" || strings.TrimSpace(input.Password) == "" {
		return LoginOutput{}, pkgerrors.InvalidParams().WithHTTPStatus(422).WithMessage("账号和密码不能为空")
	}
	...
	if err := s.dbRepo.Default().WithContext(ctx).
		...
		if err == gorm.ErrRecordNotFound {
			...
			return LoginOutput{}, pkgerrors.Unauthorized().WithMessage("账号或密码错误").WithCode("invalid_credentials")
		}
		return LoginOutput{}, pkgerrors.Internal().WithCause(err)
	}

	if !admin.CanLogin() {
		s.writeLoginLog(ctx, admin.ID, admin.Account, false, "账号已被禁用")
		return LoginOutput{}, pkgerrors.Forbidden().WithMessage("账号已被禁用").WithCode("account_disabled")
	}
	if admin.RoleID != "" && (admin.Role == nil || !admin.Role.IsActive()) {
		s.writeLoginLog(ctx, admin.ID, admin.Account, false, "角色不可用")
		return LoginOutput{}, pkgerrors.Forbidden().WithMessage("角色不可用").WithCode("role_unavailable")
	}
	...
	if err := s.dbRepo.Default().WithContext(ctx).
		...
		Updates(map[string]any{
			"last_login_at": now,
			"last_login_ip": truncateLoginIP(reqctx.GetRequestMetaFromContext(ctx).ClientIP),
			"login_count":   gorm.Expr("login_count + 1"),
		}).Error; err != nil {
		return LoginOutput{}, pkgerrors.Internal().WithCause(err)
	}
	admin.LastLoginAt = &now
	admin.LastLoginIP = truncateLoginIP(reqctx.GetRequestMetaFromContext(ctx).ClientIP)
	admin.LoginCount++
...
func (s *AuthService) RefreshToken(ctx context.Context, input RefreshTokenInput) (RefreshTokenOutput, error) {
	if s.tokenManager == nil {
		return RefreshTokenOutput{}, pkgerrors.ServiceUnavailable().WithMessage("令牌管理器未配置")
	}

	refreshToken := strings.TrimSpace(input.RefreshToken)
	if refreshToken == "" {
		return RefreshTokenOutput{}, pkgerrors.InvalidParams().WithHTTPStatus(422).WithMessage("刷新令牌不能为空")
	}

	claims, err := s.tokenManager.ParseRefreshToken(refreshToken)
	if err != nil {
		return RefreshTokenOutput{}, pkgerrors.Unauthorized().WithMessage("刷新令牌无效").WithCode("invalid_refresh_token").WithCause(err)
	}
	if claims.UserType != "console" || claims.AdminID == "" {
		return RefreshTokenOutput{}, pkgerrors.Unauthorized().WithMessage("刷新令牌无效").WithCode("invalid_refresh_token")
	}
	...
	if err != nil {
		return RefreshTokenOutput{}, pkgerrors.Internal().WithCause(err)
	}
...
func (s *AuthService) Logout(_ context.Context, input LogoutInput) error {
	if s.tokenManager == nil {
		return pkgerrors.ServiceUnavailable().WithMessage("令牌管理器未配置")
	}
...
func (s *AuthService) ChangePassword(ctx context.Context, input ChangePasswordInput) error {
	...
	if err := s.dbRepo.Default().WithContext(ctx).
		...
		First(&admin).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return pkgerrors.NotFound().WithMessage("管理员不存在")
		}
		return pkgerrors.Internal().WithCause(err)
	}

	if err := bcrypt.CompareHashAndPassword([]byte(admin.Password), []byte(input.OldPassword)); err != nil {
		return pkgerrors.Unauthorized().WithMessage("原密码不正确").WithCode("invalid_credentials")
	}
...
func (s *AuthService) UpdateCurrentAdmin(ctx context.Context, input UpdateCurrentAdminInput) (*model.ConsoleAdmin, error) {
	...
	if adminID == "" {
		return nil, pkgerrors.InvalidParams().WithHTTPStatus(422).WithMessage("管理员ID不能为空")
	}
