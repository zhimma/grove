...
func NewRoleService(dbRepo database.Repo, enforcer *pkgcasbin.Enforcer, runtimePermission ...*RuntimePermissionCatalog) *RoleService {
...
func (s *RoleService) CreateRole(ctx context.Context, in CreateRoleInput) (*Role, error) {
	...
	if name == "" || code == "" {
		return nil, pkgerrors.InvalidParams().WithHTTPStatus(422).WithMessage("角色名称和角色编码不能为空")
	}
	if !isRoleStatusValid(in.Status) && in.Status != 0 {
		return nil, pkgerrors.InvalidParams().WithHTTPStatus(422).WithMessage("角色状态不合法")
	}
...
func (s *RoleService) UpdateRole(ctx context.Context, in UpdateRoleInput) (*Role, error) {
	...
	if current.IsSuper {
		return nil, pkgerrors.Forbidden().WithMessage("系统角色不允许修改")
	}
	...
	if in.Name != nil {
		name := strings.TrimSpace(*in.Name)
		if name == "" {
			return nil, pkgerrors.InvalidParams().WithHTTPStatus(422).WithMessage("角色名称不能为空")
		}
		updates["name"] = name
	}
	if in.Code != nil {
		code := strings.TrimSpace(*in.Code)
		if code == "" {
			return nil, pkgerrors.InvalidParams().WithHTTPStatus(422).WithMessage("角色编码不能为空")
		}
...
func (s *RoleService) DeleteRole(ctx context.Context, in DeleteRoleInput) error {
	...
	if role.IsSuper {
		return pkgerrors.Forbidden().WithMessage("系统角色不允许删除")
	}
...
func (s *RoleService) SetRolePermissions(ctx context.Context, in SetRolePermissionsInput) error {
	...
	if role.IsSuper {
		return pkgerrors.Forbidden().WithMessage("系统角色不允许修改接口权限")
	}
	if s.enforcer == nil {
		return nil
	}
...
func (s *RoleService) validateAPIIdentifiers(keys []string) error {
	...
	if s.runtimePermission == nil {
		return pkgerrors.ServiceUnavailable().WithMessage("运行时接口权限清单未加载")
	}
	...
	if len(invalid) > 0 {
		return pkgerrors.InvalidParams().WithHTTPStatus(422).WithMessage("存在未注册的接口权限标识: " + strings.Join(invalid, ", "))
	}
	return nil
}
...
func (s *RoleService) SetRoleMenus(ctx context.Context, in SetRoleMenusInput) error {
	...
	if role.IsSuper {
		return pkgerrors.Forbidden().WithMessage("系统角色不允许修改菜单权限")
	}
...
func (s *RoleService) loadRole(ctx context.Context, roleID string) (*model.ConsoleRole, error) {
	...
	if err := s.dbRepo.Default().WithContext(ctx).First(&role, "id = ?", strings.TrimSpace(roleID)).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, pkgerrors.NotFound().WithMessage("角色不存在")
		}
		return nil, pkgerrors.Internal().WithCause(err)
	}
	return &role, nil
...
func (s *RoleService) ensureRoleCodeUnique(ctx context.Context, excludeID, code string) error {
	...
	if count > 0 {
		return pkgerrors.Conflict().WithCode("role_code_exists").WithMessage("角色编码已存在")
	}
	return nil
}
