...
func (s *SystemConfigService) CreateConfig(ctx context.Context, in CreateSystemConfigInput) (*model.SystemConfig, error) {
	...
	if record.ConfigGroup == "" || record.ConfigKey == "" {
		return nil, pkgerrors.InvalidParams().WithHTTPStatus(422).WithMessage("配置分组和配置键不能为空")
	}
	if record.Name == "" {
...
func (s *SystemConfigService) UpdateConfigByID(ctx context.Context, in UpdateSystemConfigByIDInput) (*model.SystemConfig, error) {
	...
	if !record.IsEditable {
		return nil, pkgerrors.Forbidden().WithMessage("系统配置为只读，不允许修改")
	}
	value := strings.TrimSpace(in.Value)
...
func validateSystemConfigValue(valueType, value string) error {
	...
	case "array":
		...
		if err := json.Unmarshal([]byte(value), &target); err != nil {
			return pkgerrors.InvalidParams().WithHTTPStatus(422).WithMessage("配置值必须是合法的 JSON 数组")
		}
	case "int":
		_, err := (&model.SystemConfig{Value: value, ValueType: "int"}).IntValue()
		if err != nil {
			return pkgerrors.InvalidParams().WithHTTPStatus(422).WithMessage("配置值必须是整数")
		}
	case "bool":
		_, err := (&model.SystemConfig{Value: value, ValueType: "bool"}).BoolValue()
		if err != nil {
			return pkgerrors.InvalidParams().WithHTTPStatus(422).WithMessage("配置值必须是布尔值")
		}
	case "json":
		var target any
		if err := json.Unmarshal([]byte(value), &target); err != nil {
			return pkgerrors.InvalidParams().WithHTTPStatus(422).WithMessage("配置值必须是合法的 JSON")
		}
	}
	return nil
}
